...
class RNN(object):
  ...
  def set_data(self,x,y,batch_size):
  ...
  def compile(self):
    ...
    # an iscalar to index every mini-batch
    i = T.iscalar('i')
    
    bs = self.batch_size
    #x_shrd = theano.shared(self.x_train)
    #y_shrd = theano.shared(self.y_train)

    givens = {self.X: self.x_train[i*bs:(i+1)*bs],
              self.Y: self.y_train[i*bs:(i+1)*bs]}
    train_ = theano.function(inputs=[i], 
                            outputs=self.error,
                            updates=self.updates,
                            givens=givens)
